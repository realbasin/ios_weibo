addEventListener("load",function(){
	...
	top_left.onclick = function(){
		plus.webview.close("my_weibo.html","pop-out");
	}
	var top_right = document.getElementById('top_right');
	top_right.onclick = function(){
		var actionbuttons=[{title:"刷新"},{title:"返回首页"}];
	...
	stopDef();
	getDataByUrl(url,{count:'50',uid:'2264734132'},successFN,errorFN);
},false);
...
//					var uid = obj.statuses[i].user.id;
					...
					read.className = 'read';
					top_1.appendChild(read);
					var read_1 = document.createElement('div');
					read_1.innerText = weibo.attitudes_count;
					read.appendChild(read_1);
					var read_2 = document.createElement('div');
					read_2.innerText = '阅读';
					read.appendChild(read_2);
					...
					down.onclick = function(){
						if(weibo.favorited == true){
							var actionbuttons=[{title:"取消收藏"},{title:"设为好友可见"},{title:"设为自己可见"},{title:"置顶"},{title:"推广"},{title:"删除",style:'destructive'}];
						}else{
							var actionbuttons=[{title:"收藏"},{title:"设为好友可见"},{title:"设为自己可见"},{title:"置顶"},{title:"推广"},{title:"删除",style:'destructive'}];
						}
						var actionstyle={cancel:"取消",buttons:actionbuttons};
						caBubble();
						plus.nativeUI.actionSheet(actionstyle, function(e){
